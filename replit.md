...
- **File Upload**: `ArchivosUploadService.postApiArchivosUpload(file)` - uploads image via multipart form data, returns `{ id }`. The returned `id` (archivoId) is stored in state and sent when creating/editing events.
